...
For an overview of the per-product position limits, please refer to the ‘Rounds’ section on ‣. The position limits are listed per round.
...
See the [Trading glossary page](https://imc-prosperity.notion.site/trading-glossary) for a more elaborate explanation of order execution in financial markets.

## Position Limits

Just like in the real world of trading, there are position limits, i.e. limits to the size of the position that the algorithm can trade into in a single product. These position limits are defined on a per-product basis, and refer to the absolute allowable position size. So for a hypothetical position limit of 10, the position can neither be greater than 10 (long) nor less than -10 (short). On the Prosperity exchange, this position limit is enforced by the exchange. If at any iteration, the player’s algorithm tries to send buy (sell) orders for a product with an aggregated quantity that would cause the player to go over long (short) position limits if all orders on a side would be fully executed, all orders will be rejected automatically. 

<aside>
📃

**Example 1**: say the position limit in product X is 30 and the current position is -5, then any aggregated buy order volume exceeding 30 - (-5) = 35 would result in an order rejection. However, an order with volume/quantity 35 itself is perfectly legal!
...
- Position limit: 10
- Current position: 9
- Your orders: [(buy, volume 8), (sell, volume 8)]

You could argue this set of orders should be allowed since 8-8=0, hence you end up with no net position change. However, this is *not* a legal set of orders, since you a priori have no guarantee of both sides being executed properly.

For example, if your buy order executes but your sell order doesn’t, you end up with a position of 9+8=17, which is not legal (position limit = 10). To avoid this scenario, we check if both your aggregated buy orders and aggregated sell orders sepatately don’t make you violate your position limit.

</aside>

For an overview of the per-product position limits, please refer to the ‘Rounds’ section on the wiki. The position limits are listed per round.

<aside>
⚠️

**Warning**: If the sum of volume in your orders makes you exceed your position limit, *all orders* are cancelled. For example, if your

- Position limit = 3
- Position = 0
- Sell volume in order book = 5

Then both of these sets of orders would be cancelled in full:

- [(buy, volume=1), (buy, volume=1), (buy, volume=1), (buy, volume=1)]
- [(buy, volume=4)]

Since your buy orders would be executed simultaneously, both scenarios would execute fully and immediately get you a position of 4, which is over your position limit of 3. A similar example can be made for sell quotes.
