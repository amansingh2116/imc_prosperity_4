"""
IMC Prosperity Round 3 - Trader v12

Changes from v10:
1. HYDROGEL_PACK stays very close to the official-positive v10 logic.
2. VEV_5400 keeps the smile-fit fair value, but becomes more passive:
   - higher threshold to cross the ask
   - stronger passive threshold before posting inside the spread
   - smaller max inventory and earlier inventory bleed
3. No direct VELVETFRUIT_EXTRACT trading and no v11-style hedging.
"""
...
class Trader:
    ...
    def _trade_5400(self, od: OrderDepth, pos: int, fair: float, ts: int) -> List[Order]:
        ...
        # Sell more quickly if the market bid has converged back to our fair.
        exit_edge = bb - fair
        if running > 0 and exit_edge >= OPT_EXIT_EDGE:
            qty = _tiered_size(exit_edge + 1.0, 4, 6, 8)
            qty = min(qty, running)
            qty = clamp_sell(sym, running, qty)
            if qty > 0:
                orders.append(Order(sym, int(bb), -qty))
        ...
        # Very strong signal only: cross the ask.
        take_edge = fair - ba
        if running < target_pos and take_edge >= OPT_TAKE_EDGE:
            qty = _tiered_size(take_edge, 3, 5, 7)
        ...
        # Default mode: post inside the spread and let the market come to us.
        if running < target_pos:
            post_px = int(bb) + 1 if int(bb) + 1 < int(ba) else int(bb)
            post_edge = fair - post_px
            if post_edge >= OPT_POST_EDGE:
                qty = _tiered_size(post_edge, 2, 3, 5)
                qty = min(qty, target_pos - running)
                qty = clamp_buy(sym, running, qty)
                if qty > 0:
